...
import { fromEvent, pipe, map, combine } from "callbag-basics";
 ...
 */
addFilter(
	"carbon-fields.conditional-display-post-parent.classic",
 ...
 */
addFilter(
	"carbon-fields.conditional-display-post-parent.gutenberg",
	"carbon-fields/metaboxes",
	() => {
		...
		return pipe(
			...
			map(([postId, postTypeSlug, parentId]) => {
				...
				const items =
					getEntityRecords("postType", postTypeSlug, {
						per_page: -1,
						exclude: postId,
